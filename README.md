...
- **OpenSearch**: rolled up to Điều, not a literal 1:1 mirror of every `document_node` row — Khoản/Điểm are nested inside their Điều document rather than indexed as independent documents, for better BM25 statistics and more citable hits (see [docs/database-design.md §3](docs/database-design.md#3-opensearch-search-index)). A CDC event still triggers a near-real-time re-render-and-upsert of the affected Điều, just not a byte-for-byte mechanical copy.

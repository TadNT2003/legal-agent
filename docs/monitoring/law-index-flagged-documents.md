...
**"Văn bản gốc" link now captured too (2026-07-31)**, closing the follow-up originally deferred above. Investigated live: the "Văn bản gốc" tab exists on *every* document (not just these 8) and lists 1+ scanned PDF file(s). The filename is readable directly from the DOM (`.ant-list-item` text, after expanding the AntD Collapse header) — no href in the DOM, no network-response interception needed. The actual download URL is deterministic once the filename is known: `https://vbpl-bientap-gateway.moj.gov.vn/api/qtdc/public/doc/minio/buckets/vbpl/{internalId}/{filename}/download` (a public, unauthenticated Ministry-of-Justice-operated MinIO gateway; `{internalId}` is vbpl.vn's own document id, already available via `extractVbplInternalId`) — confirmed live across 3 documents with 3 different filename styles, and every constructed URL verified to actually resolve (200, real PDF bytes, correct `content-disposition`).

**Implemented:** `VbplClientService.fetchDocument()` now loads a 4th tab (`?tabs=hien-thi-pdf`), clicks the file-list's collapse header, and extracts filenames via the new `extractOriginalDocumentFilenames()`. `vbpl.parser.ts`'s new `buildOriginalDocumentUrl(internalId, filename)` constructs the real URL; `parseVbplPage` populates `ParsedVbplDocument.originalDocumentUrls` from it. New `document.original_document_urls` column (`text[]`, migration `0004_cloudy_warhawk.sql`) — captured for every document, not gated on whether `fullText` is null, per explicit scope decision (cost: one more throttled page load per document on every crawl, ~25% slower). Folded into `computeContentVersion`'s hash so a change here also triggers a re-write on re-sync, matching §5's `issuingBody` precedent. Unit tests added in `vbpl.parser.spec.ts`. Verified live: both a no-`fullText` document (`106/2016/QH13`, id 101890) and a normal document with real body text (`149/2025/QH15`, id 186981) now correctly populate `original_document_urls` alongside their (respectively null/real) `fullText`.

...
**Current corpus-wide state, 2026-08-11:** 87 documents still carry a `dedupeOrdinal` uniqueness-backstop suffix (down from 125 after [§18](#18-vbplvns-digitized-text-sometimes-garbles-the-dđ-ordinal-pair-specifically)/[§19](#19-some-older-documents-use-a-hyphen-instead-of-a-period-as-the-khoản-separator)'s fixes, down from 591 at the start of the 2026-08-08 duplicate-path push, itself down from 636 at [§12](#12-duplicate-document_node-paths--phase-0-of-the-opensearch-projector-plan-three-distinct-causes)'s initial Phase 0 audit). Real `(document_id, path, node_type)` collisions have held at 0 throughout every pass — see [§17](#17-a-root-level-điềuchương-whose-ordinal-duplicates-an-existing-sibling-is-a-duplicatedre-attached-block-not-a-genuine-collision--partial-fix-for-the-nested-list-restart-bucket) and [§18](#18-vbplvns-digitized-text-sometimes-garbles-the-dđ-ordinal-pair-specifically)/[§19](#19-some-older-documents-use-a-hyphen-instead-of-a-period-as-the-khoản-separator) for the current breakdown of what's left.
...
| [18](#18-vbplvns-digitized-text-sometimes-garbles-the-dđ-ordinal-pair-specifically) | vbpl.vn's digitized text garbles the "d)"/"đ)" ordinal pair specifically (drops one, duplicates the other) | ✅ Resolved | 23+ documents investigated, corpus-wide fix | Backstop-suffixed document count: 125 → 87 |
| [19](#19-some-older-documents-use-a-hyphen-instead-of-a-period-as-the-khoản-separator) | Some older (pre-1990s) documents use a hyphen instead of a period as the Khoản separator ("2-" not "2.") | ✅ Resolved | 1 confirmed (73-CP); broadened corpus-wide | Included in the 125 → 87 drop above |
...
- **Amendment-annotation-adjacent Khoản/Điểm collisions** — amendment content for a *different* Điều's topic gets linearized right after a real Điều's own Khoản/Điểm list, each correctly preceded by a genuine `"Điều khoản được sửa đổi, bổ sung"` annotation, colliding with the enclosing Điều's real numbering. **Fixed:** a Khoản/Điểm immediately following this annotation is checked against its current siblings and only suppressed if it would actually collide (`wouldCollideWithSibling`, shared across both node types). **Broadened 2026-08-11** (investigating the ~40-document unclassified backlog below): vbpl.vn has a sibling annotation for a REPEALED clause, `"Điều khoản được bãi bỏ"`, that produces the identical collision shape — confirmed on `08/2012/QH13`, where a repealed Điểm a ("Trường cao đẳng") gets linearized after a later Khoản's own real Điểm list and collides with that Khoản's real Điểm a. `AMENDMENT_ANNOTATION_LINES` (renamed from the singular `AMENDMENT_ANNOTATION_LINE`) is now a `Set` of both trigger phrases, reusing the exact same `wouldCollideWithSibling` suppression logic — `08/2012/QH13` now fully clean (was 1 collision, now 0).
...
- **Real content duplicated with no recoverable signal at all** — the same ordinal reused a 2nd time for genuinely different content, with no annotation, heading, quote marker, or even a blank line distinguishing the two occurrences. Permanent backstop-only; not expected to ever be fixable by a line-based parser. Confirmed clauses: `117/2020/NĐ-CP` (Chương III, Điều 107, Khoản 3, Điểm b — two consecutive `"b)"` items with the amendment annotation misplaced after both, annotating the following `"c)"` instead); `168/2024/NĐ-CP` (Chương II, Mục 2, Điều 16, Khoản 2, Điểm đ — skips `"d)"`, reuses `"đ)"`; Mục 5, Điều 40, Khoản 3, Điểm c and Điểm đ — a garbled `a) b) c) đ) đ) c)` sequence); `02/2026/NĐ-CP` (Chương II, Điều 9, Khoản 1, Điểm d — reuses `"d)"` for two consecutive items instead of `"d)"` then `"đ)"`); `133/2016/NĐ-CP` (Điều 4, Khoản 2, Điểm a — a "g)" item reused as a second "a)" instead, isolated, not part of the đ/d family below); `63/2015/NĐ-CP` and `11/2026/NQ-CP` (a duplicate `"b)"`/`"a)"` respectively with genuinely different content and no marker at all); `02/2011/QH13` (Điều 31, Khoản 2, Điểm i — a duplicate `"i)"`, the last item in its list, so there's no following letter to confirm a gap); `23/2012/QH13` (a duplicate `"b)"` with `"đ)"` present and correctly placed elsewhere in the same list, so not part of the §18 pair either).
- **A more specific, different cause found the same day: vbpl.vn's own digitized text is missing an Điều/Khoản heading entirely.** `02/2026/NĐ-CP`'s stored `fullText` jumps directly from "Điều 13" to "Điều 15" — Điều 14's real content is present but unlabeled, silently absorbed into Điều 13, and its own Khoản 1/2 collide with Điều 13's real Khoản 1/2 (Chương II, Điều 13, Khoản 1 and Khoản 2). Confirmed against the scanned original document, which does have Điều 14 — this is a vbpl.vn data-quality gap in their own "Nội dung" text, not a DOM-extraction bug, so there's no live-page re-scrape fix available. Architecturally different from every fix above: recovering it would mean synthesizing a node for a heading absent from the source rather than suppressing/reclassifying present content — not attempted, flagged for a scoping decision. **3 more confirmed instances, 2026-08-11** (investigating the ~40-document unclassified backlog below): `181/2026/NĐ-CP` (Điều 30, Khoản 2 heading survives only as a fragment — the leading digit and most of "kiểm soát viên" are gone, left as ". Đối với người đại diện phần vốn nhà nước, kiểm soát vi"); `105/2026/NĐ-CP` (Điều 13, Khoản 4 heading similarly truncated — "quyên và quy trình xem xét tạm dừng đóng kinh phí côn", missing "Thẩm " and its own digit); `16/2011/UBTVQH12` (Điều 18, Khoản 2 heading is missing outright — no fragment survives at all, content jumps straight from Khoản 1 to Khoản 3). All 4 (including `02/2026/NĐ-CP`) share the same architecture: not attempted, same reasoning as above.
- **The ~40-document unclassified backlog from the original 364-document audit ([§12](#12-duplicate-document_node-paths--phase-0-of-the-opensearch-projector-plan-three-distinct-causes)) and the §16-adjacent unclassified population — investigated 2026-08-11.** Broke down into 2 new fixable causes (see [§18](#18-vbplvns-digitized-text-sometimes-garbles-the-dđ-ordinal-pair-specifically), [§19](#19-some-older-documents-use-a-hyphen-instead-of-a-period-as-the-khoản-separator)) plus 3 more confirmed instances of the two already-open buckets directly above. Of the 40, 32 are now fully clean; the remaining 8 are accounted for in those two bullets, not a new mystery.

---

## 18. vbpl.vn's digitized text sometimes garbles the "d)"/"đ)" ordinal pair specifically

| | |
| --- | --- |
| **Status** | ✅ Resolved |
| **Found** | 2026-08-11, investigating the ~40-document unclassified backlog flagged in [§17](#17-a-root-level-điềuchương-whose-ordinal-duplicates-an-existing-sibling-is-a-duplicatedre-attached-block-not-a-genuine-collision--partial-fix-for-the-nested-list-restart-bucket) |
| **Docs affected** | 23+ of the 40 investigated (30 of their 46 total collisions); corpus-wide `diem` ordinal `d`/`đ` collisions dropped from 85+9=94 to 7 (the 7 remaining are unrelated genuine duplicates, not this shape — see §17) |
| **DB impact** | Backstop-suffixed document count: 125 → 87 |

**Root cause.** Vietnamese Điểm lists are alphabetically ordered with `đ` immediately following `d` (a, b, c, d, đ, e, ..., per `docs/vn-legal-document-structure.md`). Confirmed live across 23+ documents, vbpl.vn's own digitized "Nội dung" text sometimes garbles this specific pair — always in a way that leaves the OTHER member of the pair completely absent from the same Điểm list, never both correctly present alongside a collision:

- **A second "d)" where "đ)" belongs** (the dominant shape, 26 of 30 collision instances) — confirmed on `113/2025/NĐ-CP` (5 separate occurrences in one document alone): `"...d) Riêng biệt với DC. / d) Kết nối kỹ thuật để đồng bộ dữ liệu với DC. / e) Đủ năng lực..."` — both are literal ASCII `d` (U+0064), byte-inspected, not a diacritic-stripping artifact at the extraction layer (that's [§13](#13-sanitizeordinalforltree-silently-stripped-the-vietnamese-letter-đ--a-repository-layer-bug-not-a-parser-one), already fixed, and at a different layer — the repository, not the source text itself).
- **The mirror shape: a second "đ)" where "d)" belongs** (4 collision instances) — confirmed on `103/2016/NĐ-CP`: `"...c) Riêng biệt... / đ) Phòng xét nghiệm phải kín... / đ) Cửa sổ và cửa ra vào... / e) Hệ thống..."` — `"d)"` never appears at all in the list; also confirmed on `102/2016/NĐ-CP`, `04/2018/NĐ-CP`, `56/2015/NĐ-CP`.

Since the other member of the pair never appears anywhere else in the same Điểm list in either shape, this is deterministic to correct, not a guess — the content is genuinely present, just mislabeled by the source. This is architecturally different from the missing-heading gap in [§17](#17-a-root-level-điềuchương-whose-ordinal-duplicates-an-existing-sibling-is-a-duplicatedre-attached-block-not-a-genuine-collision--partial-fix-for-the-nested-list-restart-bucket): no content is missing here, only mislabeled.

**Fix.** `reclaimDDiacriticCollisions()` (`document-node.parser.ts`) runs once over the fully-built tree (needs the complete sibling list to confirm the other letter never appears — a lookahead-only check inline during parsing would misfire on the ~5 corpus-wide cases where a real `đ` legitimately follows a `d`/`d` collision). For each Điểm-bearing sibling group: a `d`/`d_2` collision with no `đ` anywhere in the list relabels the second occurrence to `đ`; a `đ`/`đ_2` collision with no `d` anywhere relabels the FIRST occurrence to `d` and the second back to plain `đ`. Left untouched (falls through to the ordinary `dedupeOrdinal` backstop) whenever the other letter is already present elsewhere in the list — confirmed necessary: naively relabeling on collision alone would misfire on documents where a real `đ`/`d` correctly follows the collision.

**Verification:** 3 new parser tests (2 shapes + 1 negative case confirming the "other letter already present" guard), 49/49 parser tests pass, 552/552 full suite (also includes §19's tests). Re-synced corpus-wide via the same no-rescrape-needed pattern as §13/§16 (raw text unchanged, only parser logic changed) — 125 documents re-processed, 0 failures.

---

## 19. Some older documents use a hyphen instead of a period as the Khoản separator

| | |
| --- | --- |
| **Status** | ✅ Resolved |
| **Found** | 2026-08-11, investigating the ~40-document unclassified backlog flagged in [§17](#17-a-root-level-điềuchương-whose-ordinal-duplicates-an-existing-sibling-is-a-duplicatedre-attached-block-not-a-genuine-collision--partial-fix-for-the-nested-list-restart-bucket) |
| **Docs affected** | 1 confirmed (`73-CP`, a pre-1990s document — 3 of its 6 collisions, `a`/`a_2`/`a_3`/`b`/`b_2`/`b_3`) |
| **DB impact** | `73-CP` fully clean (was 6 collisions, now 0); included in §18's corpus-wide 125 → 87 drop |

**Root cause.** `KHOAN_PATTERN` required a literal period (`\.`) as the ordinal separator. `73-CP` (an old Nghị định predating the modern citation/formatting convention) numbers its Khoản with a hyphen instead: `"1. Bộ Ngoại giao... / 2- Những người nước ngoài... / 3- Những đối tượng..."`. `"2-"` and `"3-"` silently failed to match, so Khoản 2 and Khoản 3's entire content — including their own `"a)"`/`"b)"` Điểm lists — got absorbed as trailing text onto the still-open Khoản 1, colliding with Khoản 1's real `"a)"`/`"b)"` the same way any other unrecognized-heading case does.

**Fix.** `KHOAN_PATTERN` broadened from `/^(\d+)([a-zđ]?)\s*\.\s*(.*)$/u` to `/^(\d+)([a-zđ]?)\s*(?:\.\s*|-\s+)(.*)$/u` — same `[.-]` precedent already used for `GROUP_MARKER_PATTERN` ([§17](#17-a-root-level-điềuchương-whose-ordinal-duplicates-an-existing-sibling-is-a-duplicatedre-attached-block-not-a-genuine-collision--partial-fix-for-the-nested-list-restart-bucket)'s outer-grouping fix). The hyphen alternative requires at least one trailing whitespace (unlike the period, which allows zero) specifically to avoid misreading an inline number range like `"3-4 người"` — never observed with a following space in real prose — as a fake Khoản header; confirmed via a dedicated negative test.

**Verification:** 2 new parser tests (the hyphen shape + the number-range negative case), included in §18's 49/49 parser / 552/552 full-suite verification run and the same corpus-wide re-sync pass.

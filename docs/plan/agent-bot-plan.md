# agent-bot (Discord legal-agent bot) implementation plan
...
The agent core and Discord bot deliberately live in **their own server, entirely separate from the scraper/ingestion server** — a different deployable process/codebase, not a module bolted onto `server/`. This repo has no workspace tooling (no root `package.json`) — `server/` is already a standalone package — so `agent-bot/` is a sibling standalone package, communicating with `server/` only over HTTP.
...
### Retrieval tools (`agent-bot/src/agent/tools.ts`)
...
## Dependencies (`agent-bot/package.json`)
...
- `npm run build` / `npm run lint` / `npm test` inside `agent-bot/` — all clean, 13 unit tests covering the tool dispatcher (including the outline-stripping behavior) and the agent loop's control flow (tool execution, error-in-tool-call handling, MAX_TOOL_ROUNDS fallback).
- Live boot test: scraper server (`server/`) running on 3000, `agent-bot` started on 3100 — confirmed clean module wiring, `GET /api/health` 200, `POST /agent/chat` 400 on empty body, graceful 500 (not a crash) when the LLM endpoint was unreachable, non-fatal handling of an invalid Discord token.

...
describe('DocumentNodeRepository (SQLite integration)', () => {
  ...
  describe('syncNodes', () => {
    it('builds nodes from full text when content changed', async () => {
      ...
      sqlite.exec(
        `INSERT INTO document (id, citation_id, title, document_type, issuing_body_id, enacted_date, effective_date, content_version, status, is_consolidated) VALUES ('doc-1', '01/2025/QH15', 'Test', 'Luật', 'ib-1', '2025-01-01', '2025-06-01', 'v1', 'con_hieu_luc', 0)`,
      );
      ...
      const nodes = sqlite
        .prepare(
          'SELECT * FROM document_node WHERE document_id = ? ORDER BY path',
        )
        .all('doc-1');
    ...
    it('skips sync when content unchanged and nodes exist', async () => {
      ...
      const nodeCountBefore = sqlite
        .prepare(
          'SELECT COUNT(*) as cnt FROM document_node WHERE document_id = ?',
        )
        .get('doc-1');

      await repo.syncNodes('doc-1', parsed, false);

      const nodeCountAfter = sqlite
        .prepare(
          'SELECT COUNT(*) as cnt FROM document_node WHERE document_id = ?',
        )
        .get('doc-1');
    ...
    it('throws when document not found', async () => {
      const parsed = makeParsedDoc('CHƯƠNG I.\nĐiều 1. Test.');
      await expect(repo.syncNodes('nonexistent', parsed, true)).rejects.toThrow(
        'not found',
      );
    });
  });
  ...
  describe('fetchDocumentInfo', () => {
    it('returns citation and title for existing document', async () => {
      ...
      sqlite.exec(
        `INSERT INTO document (id, citation_id, title, document_type, issuing_body_id, enacted_date, content_version, status, is_consolidated) VALUES ('doc-1', '01/2025/QH15', 'Test Document', 'Luật', 'ib-1', '2025-01-01', 'v1', 'con_hieu_luc', 0)`,
      );
    ...
    it('throws NotFoundException for missing document', async () => {
      await expect(repo.fetchDocumentInfo('nonexistent')).rejects.toThrow(
        'not found',
      );
    });
  });
  ...
  describe('findAllNodes', () => {
    it('returns all nodes ordered by path', async () => {
      sqlite.exec(
        `INSERT INTO document_node (id, document_id, parent_id, node_type, path, ordinal, label, heading, text_content, content_hash, valid_from, status) VALUES ('n1', 'doc-1', NULL, 'chuong', 'chuong1', '1', 'Chương I', 'Quy định chung', NULL, 'hash1', '2025-06-01', 'con_hieu_luc')`,
      );
      sqlite.exec(
        `INSERT INTO document_node (id, document_id, parent_id, node_type, path, ordinal, label, heading, text_content, content_hash, valid_from, status) VALUES ('n2', 'doc-1', 'n1', 'dieu', 'chuong1.dieu1', '1', 'Điều 1', 'Phạm vi', 'Content', 'hash2', '2025-06-01', 'con_hieu_luc')`,
      );
      sqlite.exec(
        `INSERT INTO document_node (id, document_id, parent_id, node_type, path, ordinal, label, heading, text_content, content_hash, valid_from, status) VALUES ('n3', 'doc-1', 'n1', 'dieu', 'chuong1.dieu2', '2', 'Điều 2', 'Thi hành', 'Content 2', 'hash3', '2025-06-01', 'con_hieu_luc')`,
      );
    ...
    it('filters by ordinal number', async () => {
      sqlite.exec(
        `INSERT INTO document_node (id, document_id, parent_id, node_type, path, ordinal, label, heading, text_content, content_hash, valid_from, status) VALUES ('n1', 'doc-1', 'n1p', 'dieu', 'chuong1.dieu1', '1', 'Điều 1', 'Phạm vi', 'Content', 'hash1', '2025-06-01', 'con_hieu_luc')`,
      );
      sqlite.exec(
        `INSERT INTO document_node (id, document_id, parent_id, node_type, path, ordinal, label, heading, text_content, content_hash, valid_from, status) VALUES ('n2', 'doc-1', 'n1p', 'dieu', 'chuong1.dieu2', '2', 'Điều 2', 'Thi hành', 'Content 2', 'hash2', '2025-06-01', 'con_hieu_luc')`,
      );
  ...
  describe('findNodeSubtree', () => {
    ...
    it('throws NotFoundException when nodeId not in document', async () => {
      ...
      await expect(
        repo.findNodeSubtree('doc-1', 'nonexistent'),
      ).rejects.toThrow('not found');
    });

    it('throws NotFoundException when document not found', async () => {
      await expect(repo.findNodeSubtree('nonexistent', 'n1')).rejects.toThrow(
        'not found',
      );
    });
  });

import { LawIndexService, SyncDocumentResult, SyncSummary } from './law-index.service';
import type {
  RawVbplPage,
  VbplSearchFilters,
  VbplSearchResult,
...
const mockParseVbplPage = parseVbplPage as jest.MockedFunction<typeof parseVbplPage>;
const mockParseVbplSearchPage = parseVbplSearchPage as jest.MockedFunction<typeof parseVbplSearchPage>;

const makeParsedDoc = (scope: VbplScope = 'trung-uong'): ParsedVbplDocument => ({
...
describe('LawIndexService', () => {
  ...
  describe('syncDocument', () => {
    it('syncs a trung uong document successfully', async () => {
      ...
      mockRepo.upsertDocument.mockResolvedValue({ documentId: 'doc-1', changed: true });

      const result: SyncDocumentResult = await service.syncDocument('https://vbpl.vn/doc/123');

      expect(mockClient.fetchDocument).toHaveBeenCalledWith('https://vbpl.vn/doc/123');
      ...
      expect(mockNodeRepo.syncNodes).toHaveBeenCalledWith('doc-1', parsed, true);
    ...
    it('returns early when content unchanged', async () => {
      ...
      mockClient.fetchDocument.mockResolvedValue(raw);
      mockParseVbplPage.mockReturnValue(parsed);
      mockRepo.upsertDocument.mockResolvedValue({ documentId: 'doc-1', changed: false });

      const result = await service.syncDocument('https://vbpl.vn/doc/123');

      expect(result.changed).toBe(false);
      expect(mockNodeRepo.syncNodes).toHaveBeenCalledWith('doc-1', parsed, false);
    });
  ...
  describe('syncAll', () => {
    it('crawls sitemap and syncs documents', async () => {
      mockSitemap.fetchTrungUongSitemapUrls.mockResolvedValue(['https://vbpl.vn/sitemap1.xml']);

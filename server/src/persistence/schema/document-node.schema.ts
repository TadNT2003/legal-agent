...
export const documentNode = pgTable(
  'document_node',
  {
  ...
  // Confirmed live (2026-08-07): none of document_id/parent_id/
  // superseded_by_node_id had a supporting index, so every syncNodes()
  // delete-then-reinsert paid for both the `WHERE document_id = $1` lookup
  // AND Postgres's own per-row FK-reference check on `parent_id`
  // ("SELECT 1 FROM document_node WHERE parent_id = $1 FOR KEY SHARE") as a
  // full sequential scan of the whole table — confirmed via pg_stat_activity
  // catching that exact query mid-flight, still running after 3+ minutes for
  // one 1,636-node document. These three indexes make both the lookup and
  // the FK check O(log n) instead of O(n).
  (table) => [
    index('document_node_document_id_idx').on(table.documentId),
    index('document_node_parent_id_idx').on(table.parentId),
    index('document_node_superseded_by_node_id_idx').on(
      table.supersededByNodeId,
    ),
  ],
);

...
**Formerly one umbrella `server/src/law/` module (`law.module.ts`, now deleted) — flattened into three top-level modules, one at a time (see git history/branch `migration/module-reorganize`).** Each is registered directly in `AppModule`, and their endpoints dropped the `laws`/`law` prefix (`download/`'s routes are `/downloads`, `catalog/`'s are `/catalog`, both not `/laws/...`). This flattening is now complete for workflow A. The same push has started on `law-index/`'s side too — `crawl/` is the first submodule moved there (see below); the rest (`persistence/`, `retrieve/`, `sync/`, `job-queue/`, `opensearch/`) haven't yet, so don't assume the pattern extends to them until it's noted here.
...
**Endpoints that exist on this branch** (`law-index.controller.ts`): `POST /laws/index/crawl/url` (one document — the only per-document sync endpoint here), `POST`/`PUT /laws/index/crawl/batch` (up to 100 URLs per call — present on this branch, `fix/server-resilience`; **still absent on some other branches**, e.g. the branch this file was originally written against — don't assume it's available without checking `git branch`/grepping the controller first), `POST /laws/index/crawl/all` (crawls the entire Trung ương sitemap, no tier/type filter, one long unbounded request — fine for a smoke test with `limit`, not for a targeted tier pass), `GET /laws/index/crawl/search` (read-only filtered search against live vbpl.vn, does not persist), `GET /laws/index/search` (same filters against already-synced Postgres rows). Step 3 below still recommends individual `POST /crawl/url` calls, paced in small chunks, over `/crawl/batch` even where it's available — the reliability history in §6/§11 of the monitoring log is about the underlying sequential scrape loop itself, not the transport, so it applies the same way to either endpoint shape.
...
**5. After the pass completes, heal dangling references it may have created.** `POST`/`PUT /laws/index/crawl/url` no longer heal `document_reference` rows left dangling (target not yet scraped) after every single call — that per-document heal was a full dangling-refs table scan repeated on every sync, removed as part of the scraper-resilience hardening (see `docs/plan/scraper-resilience-plan.md`). The batch/job endpoints (`POST`/`PUT /crawl/batch`, `POST /crawl/all`, `POST /crawl/search`) still heal once at the end on their own, but a pass driven by repeated individual `POST /crawl/url` calls — the pattern step 3 above actually uses — gets none of that safety net. Call `PATCH /laws/index/sync/refs/all` once after such a pass finishes to heal anything it made newly resolvable; skipping this step means references pointing at documents synced during the pass can stay dangling indefinitely.

...
**Module tree is being flattened (in progress, one submodule at a time — see git history/branch `migration/module-reorganize`): each submodule becomes its own top-level module registered directly in `AppModule`, and its endpoints drop the `laws`/`law` prefix.** `download/` and `utils/` have already moved to `server/src/download/` and `server/src/utils/` (top-level `LawDownloadModule`/`LawUtilsModule`, download's routes now under `/downloads` not `/laws/downloads`) — treat any reference below to either living under `law/` as stale. `catalog/` (and `law-index/`'s submodules) haven't moved yet, so `law.module.ts` still exists as an umbrella for it alone.

- **`download/`** (now `server/src/download/`, not under `law/` — see note above) — fetches documents from vanban.chinhphu.vn (the *only* source this module ever hits) via `VanBanChinhPhuClientService` (HTTP + cheerio HTML parsing) and `vanban-chinh-phu.parser.ts`. `LawDownloadService` orchestrates single-URL, batch (up to 100), and filtered-search downloads. `law-tier-classifier.ts` maps each document's type + issuing body to one of the 14 statutory tiers (Điều 4, Luật 64/2025/QH15); unrecognized types return an error rather than guessing, unless the caller passes `subdirOverride`.
